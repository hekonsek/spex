...
export const supportedSpexTypes = ["adr", "instruction", "format"] as const;
  ...
  async run(input: ValidateServiceInput = {}): Promise<ValidateServiceResult> {
    ...
    if (issues.length === 0) {
      ...
      if (existingTypeDirectories.length === 0) {
        issues.push(
          "Missing supported type directory in spex. Expected at least one of: adr, instruction, format.",
        );
      }
