...
async function writeProjectFixture(path, packageIds) {
    ...
    await writeFile(resolve(path, ".spex", "spex.yml"), `packages:\n${packageIds.map((packageId) => `  - ${packageId}`).join("\n")}\n`, "utf8");
}
async function configureGitHome(homePath, repositoriesRootPath) {
    await mkdir(homePath, { recursive: true });
    await writeFile(resolve(homePath, ".gitconfig"), `[user]
  name = Spex Test
...
async function createPackageRepository(repositoriesRootPath, namespace, name, files) {
...
async function updatePackageRepository(repository, files) {
    for (const [relativePath, content] of Object.entries(files)) {
...
test("build writes AGENTS.md and skips import cleanup when build file is missing", { concurrency: false }, async () => {
    ...
    try {
        ...
        await writeFile(resolve(projectPath, ".spex", "imports", "github.com", "acme", "pkg", "adr_0001.md"), "# Imported ADR\n", "utf8");
        ...
        assert.equal(await pathExists(resolve(projectPath, ".spex", "imports", "github.com", "acme", "pkg")), true);
    }
    finally {
...
test("build refreshes imported packages and removes stale package directories", { concurrency: false }, async () => {
    ...
    try {
        ...
        assert.equal(await readFile(resolve(projectPath, ".spex", "imports", gitTestHost, "acme", "pkg-one", "adr", "adr_0001.md"), "utf8"), "# Package One v1\n");
        ...
        assert.equal(await readFile(resolve(projectPath, ".spex", "imports", gitTestHost, "acme", "pkg-one", "adr", "adr_0001.md"), "utf8"), "# Package One v2\n");
        assert.equal(await pathExists(resolve(projectPath, ".spex", "imports", gitTestHost, "acme", "pkg-two")), false);
    }
    finally {
